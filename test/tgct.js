...
suite('tgtc', () => {
    test('tgtc', async () => {
        let result = await tgtc.decodeTestResult(TEST_CERT, tgtc.TEST_PUBLIC_KEY);
    ...
    test('tgtc invalid pubkey', async () => {
        let brokenKey = Object.assign({}, tgtc.TEST_PUBLIC_KEY);
        brokenKey.x = 'foobar';
        assert.isRejected(tgtc.decodeTestResult(TEST_CERT, brokenKey));
    });

    test('tgtc invalid prefix', async () => {
        assert.isRejected(tgtc.decodeTestResult('HC1:' + TEST_CERT, tgtc.TEST_PUBLIC_KEY));
    });

    test('tgtc invalid cert', async () => {
        assert.isRejected(tgtc.decodeTestResult(TEST_CERT + 'GARBAGE', tgtc.TEST_PUBLIC_KEY));
    });
});